        ...
        NavigationStack {
            ZStack {
                ...
                List {
                    ...
                    ForEach(store.entries) { entry in
                        Button {
                            draft = EntryDraft(date: entry.date, species: entry.species, notes: entry.notes)
                            editingID = entry.id
                            showingAdd = true
                        } label: {
            ...
            .sheet(isPresented: $showingAdd) {
                AddEntrySheet(draft: $draft, isEditing: editingID != nil) {
                    if let id = editingID {
                        store.updateEntry(id, date: draft.date, species: draft.species, notes: draft.notes)
                    } else {
                        store.addEntry(date: draft.date, species: draft.species, notes: draft.notes, isPro: purchases.isPro)
                    }
                    showingAdd = false
                }
            }
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("Species / Breed", text: $draft.species, axis: .vertical)
                    .accessibilityIdentifier("field_species")
                        .focused($isFocused)
                    TextField("Care Notes", text: $draft.notes, axis: .vertical)
